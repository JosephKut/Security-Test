...
async function fetchURL(targetURL, timeout = 5000) {
  return new Promise((resolve) => {
    try {
      ...
      const req    = lib.get(targetURL, { timeout, rejectUnauthorized: false }, (res) => {
        let data = "";
...
async function testDirectoryTraversal(target, siteMap) {
  const results = { name: "Directory Traversal", passed: true, issues: [] };
  const base    = target.replace(/\/$/, "");

  // Build list of base URLs to test — root + all crawled endpoints
